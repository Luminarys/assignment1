...
class MatMulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        """
        # Use not here over fixed constants to allow for chained gradients without anything going wrong
        return [
                matmul_op(output_grad, node.inputs[1], False, not node.matmul_attr_trans_B),
                matmul_op(node.inputs[0], output_grad, not node.matmul_attr_trans_A, False)
                ]
